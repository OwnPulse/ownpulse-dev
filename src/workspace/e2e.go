...
func E2EStatus(_ *config.WorkspaceConfig, _ E2EOptions) error {
	fmt.Printf("\n%s Pods:\n", info("→"))
	_ = run("kubectl", "-n", e2eNamespace, "get", "pods", "-o", "wide")

	fmt.Printf("\n%s Services:\n", info("→"))
	_ = run("kubectl", "-n", e2eNamespace, "get", "svc")

	fmt.Printf("\n%s Ingresses:\n", info("→"))
	_ = run("kubectl", "-n", e2eNamespace, "get", "ingress")
...
func e2eCreateSecrets(_ string, dryRun bool) error {
	...
	createNs := exec.Command("kubectl", "create", "namespace", e2eNamespace, "--dry-run=client", "-o", "yaml")
	...
	// Postgres secret.
	if err := applySecret(e2eNamespace, "ownpulse-postgres-secrets", map[string]string{
	...
	if err := applySecret(e2eNamespace, "ownpulse-api-secret", map[string]string{
		"DATABASE_URL":   dbURL,
...
func applySecret(namespace, name string, data map[string]string) error {
	args := []string{"create", "secret", "generic", name, "-n", namespace, "--dry-run=client", "-o", "yaml"}
	for k, v := range data {
...
func e2eDeploy(repoDir string, dryRun bool) error {
	...
	if err := run("helm", "upgrade", "--install", pgRelease,
	...
	if err := run("kubectl", "-n", e2eNamespace, "rollout", "status",
	...
	if err := run("helm", "upgrade", "--install", apiRelease,
	...
	_ = runSilent("kubectl", "-n", e2eNamespace, "wait", "--for=condition=complete",
	...
	if err := run("helm", "upgrade", "--install", webRelease,
...
func e2ePrintSummary() {
	...
	fmt.Printf("  Metrics:   kubectl -n %s port-forward svc/%s 9090:9090\n", e2eNamespace, apiFullname)
